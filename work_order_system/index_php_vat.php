...
<head>
    ...
    <title>Work Order Form - React Enhanced</title>
    <!-- Google Fonts -->
    ...
    <style>
        ...
        .react-enhanced::before {
            content: '⚛️ React Enhanced + PH VAT';
            position: absolute;
            top: -10px;
            right: 20px;
            background: #667eea;
...
<body>
        ...
        <div class="form-container" style="max-width: 900px;">
            ...
            <div class="site-header">
                ...
                <div class="alert alert-info">
                    ...
                    <strong>Enhanced with React!</strong> Enjoy real-time calculations with Philippines VAT (12%) system.
                </div>
            </div>
    ...
    <!-- Wave Background Controls -->
    <script>
        let waveVisible = true;
        let currentTheme = 'ocean';
        
        function switchWaveTheme(theme) {
            const waveContainer = document.getElementById('waveBackground');
            waveContainer.className = `wave-container waves-theme-${theme}`;
            currentTheme = theme;
            
            // Update button states
            document.querySelectorAll('.wave-theme-btn').forEach(btn => {
                btn.style.opacity = '0.7';
                btn.style.transform = 'scale(1)';
            });
            
            const activeBtn = document.querySelector(`.wave-theme-btn.${theme}`);
            if (activeBtn) {
        ...
        function toggleWaveBackground() {
            const waveContainer = document.getElementById('waveBackground');
            const toggleBtn = document.getElementById('waveToggle');
            
            if (waveVisible) {
                waveContainer.style.display = 'none';
                toggleBtn.textContent = 'Show Waves';
                waveVisible = false;
            } else {
        ...
        // Initialize wave theme
        document.addEventListener('DOMContentLoaded', function() {
            switchWaveTheme('ocean');
        });
    </script>
    ...
    <!-- Enhanced Form Handling with Wave Effects -->
    <script>
        // Same as contact functionality (enhanced with wave effects)
        document.getElementById('same_as_contact').addEventListener('change', function() {
            ...
            // Visual feedback with wave theme change
            if (isChecked) {
                switchWaveTheme('mint');
                setTimeout(() => switchWaveTheme('ocean'), 2000);
            }
        ...
        // Enhanced form submission with wave effects
        document.getElementById('workOrderForm').addEventListener('submit', function(e) {
            ...
            // Form validation
            const requiredFields = ['contact_name', 'contact_email', 'street_address', 'city_zip', 'phone'];
            let hasErrors = false;
            ...
            setTimeout(() => {
                submitBtn.innerHTML = '<i class="bi bi-check-lg me-2"></i>Create Work Order';
                submitBtn.disabled = false;
                switchWaveTheme('ocean'); // Reset theme
            }, 5000);
        });
        
        // Add polish animations with wave sync
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Subtle wave theme changes for different sections
            const workOrderSection = document.querySelector('#workOrderForm');
            if (workOrderSection) {
                workOrderSection.addEventListener('focusin', function(e) {
                    if (e.target.closest('.form-section')) {
                        // Subtle theme hints based on section
                        const section = e.target.closest('.form-section');
                        const sectionTitle = section.querySelector('.section-title')?.textContent || '';
                        
                        if (sectionTitle.includes('Contact')) {
                            // No change for contact
                        } else if (sectionTitle.includes('Bill')) {
                            // Subtle hint
                            document.body.style.filter = 'hue-rotate(10deg)';
                        } else if (sectionTitle.includes('Job')) {
                            document.body.style.filter = 'hue-rotate(20deg)';
                        }
                    }
                });
                
                workOrderSection.addEventListener('focusout', function() {
        ...
        function setTimeBasedTheme() {
            ...
            if (hour >= 6 && hour < 12) {
                ...
                theme = 'ocean'; // Afternoon
            } else if (hour >= 18 && hour < 21) {
                theme = 'sunset'; // Evening
            } else {
                theme = 'purple'; // Night
            }
            
            switchWaveTheme(theme);
        }
        
        // Optional: Set theme based on time (uncomment to enable)
        // setTimeout(setTimeBasedTheme, 2000);
    </script>
</body>
</html>